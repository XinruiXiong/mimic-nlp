...
def read_csv_auto(filepath):
    if filepath.endswith('.csv.gz') or os.path.exists(filepath + '.gz'):
        return pd.read_csv(filepath if filepath.endswith('.gz') else filepath + '.gz',
                           compression='gzip', on_bad_lines='skip', low_memory=False)
    else:
...
def read_edstays_table(edstays_table_path):
    df_edstays = read_csv_auto(edstays_table_path)
...
def read_patients_table(patients_table_path):
    df_patients = read_csv_auto(patients_table_path)
    df_patients['dod'] = pd.to_datetime(df_patients['dod'])
    return df_patients

def read_admissions_table(admissions_table_path):
    df_admissions = read_csv_auto(admissions_table_path)
...
def read_icustays_table(icustays_table_path):
    df_icu = read_csv_auto(icustays_table_path)
...
def read_triage_table(triage_table_path):
    df_triage = read_csv_auto(triage_table_path)
...
def read_diagnoses_table(diagnoses_table_path):
    df_diagnoses = read_csv_auto(diagnoses_table_path)
    return df_diagnoses

def read_vitalsign_table(vitalsign_table_path):
    df_vitalsign = read_csv_auto(vitalsign_table_path)
...
def read_pyxis_table(pyxis_table_path):
    df_pyxis = read_csv_auto(pyxis_table_path)
    return df_pyxis
...
def read_discharge_table(discharge_table_path):
